...
Under the hood it runs (OCI ADK deterministic workflow):
...
async def analyze(request: AnalyzeRequest):
    ...
    if not os.getenv("EXTRACTOR_AGENT_ENDPOINT_ID") or not os.getenv("ANALYZER_AGENT_ENDPOINT_ID"):
        raise HTTPException(
            status_code=500,
            detail="Missing EXTRACTOR_AGENT_ENDPOINT_ID or ANALYZER_AGENT_ENDPOINT_ID",
        )

    logger.info(f"Analyzing: {object_name}")

    try:
        # ── Step 1: ExtractorAgent (sync ADK → run in thread) ─
        extractor_text = await asyncio.to_thread(run_extractor, object_name)
        ...
        analysis_text = await asyncio.to_thread(run_analyzer, extractor_text)
